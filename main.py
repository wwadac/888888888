...
async def cmd_help(message: types.Message):
    ...
    help_text = (
        ...
        "3. Управление через инлайн-кнопки\n\n"
        
        "📝 <b>Примеры команд:</b>\n"
        "• <code>.топ</code> - отправит заготовленный ответ\n"
        "• <code>.скидка</code> - отправит информацию о скидках\n"
        "• <code>.контакты</code> - отправит контактную информацию\n\n"
...
async def handle_menu_commands(callback: CallbackQuery):
    ...
    commands_text = (
        ...
        "🗑️ Удалить команды\n\n"
        "<i>Команды работают так: пишешь .команда → бот отправляет ответ</i>"
    )
...
async def process_command_response(message: types.Message, state: FSMContext):
    ...
    success_text = (
        ...
        f"💬 <b>Ответ:</b>\n{response[:100]}...\n\n"
        f"<i>Теперь при написании {trigger} будет отправляться этот ответ</i>"
    )
...
async def handle_private_message(message: types.Message):
    ...
    1. Проверяет команды (начинающиеся с точки)
